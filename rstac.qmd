---
title: "Download data from a STAC API using R and rstac"
format: html
---

This tutorial walks through querying a STAC API using the [rstac](https://brazil-data-cube.github.io/rstac/) R package, and downloading data from the API using rstac or [GDAL](https://gdal.org/) (via [sf](https://github.com/r-spatial/sf). This tutorial will assume that you're already familiar with [the R language](https://cran.r-project.org/) and the [terra](https://github.com/rspatial/terra) package.
...
Because the `RSTACQuery` object is a representation of a future query, we can use other functions in rstac to change our query parameters and fields before we run a query. For instance, we can use `rstac::collections()` to update our request to query the `/collections` endpoint of the Planetary Computer API, which lists all the various [collections](https://github.com/radiantearth/stac-spec/blob/master/collection-spec/collection-spec.md) (which "describe a group of [Items](https://github.com/radiantearth/stac-spec/blob/master/item-spec/item-spec.md) that share properties and metadata", to quote the STAC Collection specification):
...
Just as before, we need to use `get_request()` to turn this query *specification* into a query *result*:

```{r}
rstac::get_request(collections_query)
```

Our result shows us a long list of collections that are available from this API! These collections are a subset of the data sets available in the [Planetary Computer data catalog](https://planetarycomputer.microsoft.com/catalog); the Planetary Computer is organized so that each collection corresponds to a distinct data set in the catalog.
...
We can see that there are 250 items inside this catalog for 2021. Collectively, these items contain all LCMAP data for the continental United States for 2021, with each item containing a number of assets ("object\[s\] that \[contain\] a URI to data associated with the Item that can be downloaded or streamed", to quote [the spec](https://github.com/radiantearth/stac-spec/blob/master/item-spec/item-spec.md#asset-object); here, assets are things like "primary land cover classification" or "a metadata object") covering a relatively small chunk of the nation.
...
As luck would have it, Ashe County is entirely contained in a single LCMAP tile. Before we can download this tile, we're going to need to [authenticate ourselves with Planetary Computer](https://planetarycomputer.microsoft.com/docs/concepts/sas/). Note that this doesn't require an account or registration, but rather is a way for Microsoft to uniquely identify your Planetary Computer session and impose rate limits if needed.
...
There are a few different ways to download these assets! First, we could use the `rstac::assets_download()` function. We'll provide the name of what assets we want to download -- here, we'll download `lcpri`, which contains the primary land cover classifications for this area:
...
To download assets using GDAL, we need to extract the URL for our assets from our STAC query results. We can get the URL for the `lcpri` asset using `rstac::assets_url()`:

```{r}
lcpri_url <- rstac::assets_url(signed_stac_query, "lcpri")
lcpri_url
```

In order for this URL to work with GDAL's virtual filesystem interface, we'll need to append `/vsicurl/` to the front of the URL:

```{r}
lcpri_url <- paste0("/vsicurl/", lcpri_url)
```

Next, we'll download our asset from this URL using `sf::gdal_utils()`, which provides access to GDAL's C++ utilities. We'll use the included interface to [gdalwarp](https://gdal.org/programs/gdalwarp.html) to both download and reproject our data, using the `t_srs` and `te` arguments to gdalwarp to control the spatial reference system and the extent of our downloaded raster:
...
This download runs a good bit faster than using `assets_download()`, and results in a smaller raster covering only our area of interest:
...
sf::gdal_utils(
  "warp",
  source = paste0("/vsicurl/", rstac::assets_url(signed_query, "lcpri")),
  destination = out_file,
  options = c(
    "-t_srs", sf::st_crs(ashe)$wkt,
    "-te", sf::st_bbox(ashe)
...
If we wanted to download multiple assets, we'd need to run that `gdal_utils()` function for each asset URL we wanted to download. For instance, to download both `lcpri` and `lcsec` (the primary and secondary land cover classifications, respectively), we could run:

```{r}
vapply(
  paste0("/vsicurl/", rstac::assets_url(signed_query, c("lcpri", "lcsec"))),
  function(asset_url) {
    out_file <- tempfile(fileext = ".tif")
    sf::gdal_utils(
      "warp",
...
We barely need to change our code at all to download all 20 `lcpri` assets from these items using `assets_download()`. All we need to do is add `overwrite = TRUE`, to make sure rstac overwrites the `lcpri` tile that we downloaded earlier -- otherwise, we wouldn't need to change a thing:
...
```{r}
lcpri <- list.files(
  file.path(tempdir(), "lcmap", "CU", "V13"),
  pattern = "LCPRI.tif",
  recursive = TRUE,
  full.names = TRUE
) |> 
...
Or alternatively, we can use our `gdal_utils()` function -- without any edits -- to download and merge these files together:
